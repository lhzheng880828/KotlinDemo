 ...
 *Date:19-3-7
 */
class Lesson08_GenericType {

    companion object {
        @Volatile private var INSTANCE: Lesson08_GenericType? = null

        fun getInstance(): Lesson08_GenericType =
                INSTANCE ?: synchronized(this){
                    INSTANCE
                            ?: Lesson08_GenericType().also { INSTANCE = it }
                }
    }
    ...
    //(2)定义泛型类型变量，可以完整地写明类型参数，如果编译器可以自动推定类型参数，也可以省略类型参数。
    ...
    //(3)在调用泛型函数时，如果可以推断出类型参数，可以省略泛型参数。
    //以下实例创建了泛型函数 doPrintln，函数根据传入的不同类型做相应处理：

    fun testGenericType2(){
        val age = 23
        val name = "runoob"
        val bool = true
    ...
    * 假如类型定义为 Foo<in T> , 其中 T 是一个反向协变的类型参数, Foo<> 等价于 Foo<inNothing> . 它表示, 当 T 未知时,
    你不能安全地向 Foo<> 写入 任何东西.
    * 假如类型定义为 Foo<T> , 其中 T 是一个协变的类型参数, 上界(upper bound)为 TUpper , 对于读取值的场合, Foo<*> 等价于
    Foo<out TUpper> , 对于写入值的场合, 等价于 Foo<in Nothing> .
